...
app.get('/api/consignments', (req, res) => {
  ...
  res.json(store.consignments.filter(c => { const p = store.permissions[c.id] || {}; return p[orgId] === 'owner' || p[orgId] === 'viewer'; }));
});
app.post('/api/consignments', (req, res) => {
  ...
  const c = { id: genId(), ucr, commercialInvoiceNo: commercialInvoiceNo || '', exportDeclarationNo: exportDeclarationNo || '', description: description || '', creatorOrgId, creatorOrgName: org.name, createdAt: now(), documentCount: 0 };
...
app.post('/api/permissions/share', (req, res) => {
  ...
  if (isPeer && peerWs?.readyState === WebSocket.OPEN) {
    ...
    peerWs.send(JSON.stringify({ type: 'SHARE_CONSIGNMENT', consignment, documents: docsToSend, permissions: store.permissions[consignmentId], docPermissions: Object.fromEntries(docsToSend.map(d => [d.id, store.docPermissions[d.id] || {}])) }));
  }
  res.json({ success: true });
});
...
app.listen(PORT, () => { console.log(`[${NODE_NAME}] HTTP on http://localhost:${PORT} | WS on ws://localhost:${WS_PORT} | Peer: MANUAL`); });
